...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    flag=False
    if flag:
        seg_old_spacing_softmax=seg_old_spacing.copy()
    #############################
...
######LSL modify##
    if flag:
        np_out_fname = out_fname[:-7] + '.npy'
        np_label_out_fname = out_fname[:-7] + 'l.npy'
        buf = out_fname
        sub = "/"
        index = [substr.start() for substr in re.finditer(sub, buf)]

        case_name = buf[index[-1] + 1:]
        title = buf[:index[-2]]
        gt_name = title + '/labelsTr/' + case_name
        label = sitk.ReadImage(gt_name)
        label = sitk.GetArrayFromImage(label)
        crop_label = label[bbox[0][0]:bbox[0][1],
                     bbox[1][0]:bbox[1][1],
                     bbox[2][0]:bbox[2][1]]
        np.save(np_out_fname, seg_old_spacing_softmax)
        np.save(np_label_out_fname, crop_label)
    #########################
